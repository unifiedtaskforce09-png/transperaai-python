...
if __name__ == '__main__':
    ...
    if not os.environ.get("GOOGLE_API_KEY"):
        ...
        try:
            ...
            # 2. Define a batch of prompts
            prompts_to_process = [
                "Explain the concept of quantum entanglement in simple terms.",
                "What are the top 5 benefits of a Mediterranean diet?",
                "Write a short, uplifting poem about the sunrise.",
                "Summarize the plot of the movie 'Inception'.",
            ]

            print(f"Processing a batch of {len(prompts_to_process)} prompts...")

            # 3. Process the batch (will try async batch mode first)
            start_time = time.time()
            responses = client.process_batch(prompts_to_process)
            end_time = time.time()
